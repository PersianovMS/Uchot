Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ПродолжительностьПериода.Использование И ЗначениеЗаполнено(ПродолжительностьПериода.Значение)
		...
		Если ЗначениеЗаполнено(Дата41.Значение) Тогда
			ТекДата = ?(ТипЗнч(Дата41.Значение)= Тип("Дата"), Дата41.Значение, Дата41.Значение.Дата);
			Дата11.Значение = ?(ТипСрока = "День",
					ТекДата - 3 * ИнтервалМеждуПериодами.Значение * СекВДне,
					ДобавитьМесяц(ТекДата, -3 * ИнтервалМеждуПериодами.Значение));
		ИначеЕсли ЗначениеЗаполнено(Дата42.Значение) Тогда
			ТекДата = ?(ТипЗнч(Дата42.Значение)= Тип("Дата"), Дата42.Значение, Дата42.Значение.Дата);
			Дата11.Значение = ?(ТипСрока = "День",
					ТекДата - (3 * ИнтервалМеждуПериодами.Значение + ПродолжительностьПериода.Значение) * СекВДне,
					ДобавитьМесяц(ТекДата, -ПродолжительностьПериода.Значение - 3 * ИнтервалМеждуПериодами.Значение)) + 1;
		КонецЕсли;
